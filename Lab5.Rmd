...
We model a continuous outcome (`Sepal.Length`) as a linear function of `Sepal.Width` using the **iris** dataset.
...
where  
- \(y_i\) = response (Sepal.Length)  
- \(x_i\) = predictor (Sepal.Width)  
...
lm_multi <- lm(Sepal.Length ~ Sepal.Width + Petal.Length, data = iris)

# Model summary
summary(lm_multi)

# Predictions for new combinations of hp and wt
new_multi <- data.frame(Sepal.Width = c(2.5, 3.0, 3.5),
                        Petal.Length = c(1.0, 4.0, 5.5))
...
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length, color = Petal.Length)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", formula = y ~ x + Petal.Length, se = FALSE) +
  labs(title = "Multiple Regression: Sepal.Length ~ Sepal.Width + Petal.Length")
